...
#  $agents_package_installer_prefix_dir => The Dynatrace Agents package will be installed into the directory
#                                          $agents_package_installer_prefix_dir/dynatrace-$major-$minor-$rev,
#                                          where $major, $minor and $rev are given by the installer. A symbolic link to the
#                                          actual installation directory will be created in $agents_package_installer_prefix_dir/dynatrace.
#  $agents_package_installer_file_name  => The file name of the Dynatrace Agents installer in the module's files directory.
#  $agents_package_installer_file_url   => A HTTP, HTTPS or FTP URL to the Dynatrace Agents installer in the form
...
#  $collector_installer_prefix_dir => The Dynatrace Collector will be installed into the directory
#                                     $collector_installer_prefix_dir/dynatrace-$major-$minor-$rev,
#                                     where $major, $minor and $rev are given by the installer. A symbolic link
#                                     to the actual installation directory will be created in $collector_installer_prefix_dir/dynatrace.
#  $collector_installer_file_name  => The file name of the Dynatrace Collector installer in the module's files directory.
#  $collector_installer_file_url   => A HTTP, HTTPS or FTP URL to the Dynatrace Collector installer in the form
#                                     (http|https|ftp)://[user[:pass]]@host.domain[:port]/path.
...
#  $collector_server_port          => The port on the Dynatrace Server the Collector shall connect to.
#                                     Use either 6698 (non-SSL) or 6699 (SSL).
...
#  $memory_analysis_server_installer_prefix_dir => The Dynatrace Memory Analysis Server will be installed into the directory
#                                                  $memory_analysis_server_installer_prefix_dir/dynatrace-$major-$minor-$rev,
#                                                  where $major, $minor and $rev are given by the installer. A symbolic link
#                                                  to the actual installation directory will be created in
#                                                  $memory_analysis_server_installer_prefix_dir/dynatrace.
#  $memory_analysis_server_installer_file_name  => The file name of the Dynatrace Memory Analysis Server installer in
#                                                  the module's files directory.
#  $memory_analysis_server_installer_file_url   => A HTTP, HTTPS or FTP URL to the Dynatrace Memory Analysis Server installer
#                                                  in the form (http|https|ftp)://[user[:pass]]@host.domain[:port]/path.
#  $memory_analysis_server_server_port          => The port where the Dynatrace Memory Analysis Server shall listen
#                                                  for the Dynatrace Server.
...
#  $server_installer_prefix_dir    => The Dynatrace Server will be installed into the directory
#                                     $server_installer_prefix_dir/dynatrace-$major-$minor-$rev,
#                                     where $major, $minor and $rev are given by the installer. A symbolic link to
#                                     the actual installation directory will be created in $server_installer_prefix_dir/dynatrace.
#  $server_installer_file_name     => The file name of the Dynatrace installer in the cookbook's files directory.
#  $server_installer_file_url      => A HTTP, HTTPS or FTP URL to the Dynatrace installer in the form
#                                     (http|https|ftp)://[user[:pass]]@host.domain[:port]/path.
#  $server_license_file_name       => The file name of the Dynatrace License in the cookbook's files directory.
#  $server_license_file_url        => A HTTP, HTTPS or FTP URL to the Dynatrace License in the form
#                                     (http|https|ftp)://[user[:pass]]@host.domain[:port]/path.
#  $server_collector_port          => The port where the Dynatrace Server shall listen for Collectors.
#                                     Use either 6698 (non-SSL) or 6699 (SSL).
#  $server_embedded_collector_port => The port for embedded collector.
#  $server_pwh_connection_hostname =>
#  $server_pwh_connection_port     =>
#  $server_pwh_connection_dbms     => The DBMS type of the Performance Warehouse. Possible values are 'embedded'
#                                     (not suitable for production systems), 'db2', 'oracle', 'postgresql', 'sqlazure', 'sqlserver'.
...
#  $wsagent_package_installer_prefix_dir => The Dynatrace WebServer Agent will be installed into the directory
#                                           $wsagent_package_installer_prefix_dir/dynatrace-$major-$minor-$rev,
#                                           where $major, $minor and $rev are given by the installer. A symbolic link
#                                           to the actual installation directory will be created in
#                                           $wsagent_package_installer_prefix_dir/dynatrace.
...
#  $wsagent_package_installer_file_url   => A HTTP, HTTPS or FTP URL to the Dynatrace Web Server Agent installer
#                                           in the form (http|https|ftp)://[user[:pass]]@host.domain[:port]/path.
#
#  $host_agent_installer_prefix_dir => The Dynatrace Host Agent will be installed into the directory
#                                      $host_agent_installer_prefix_dir/dynatrace-$major-$minor-$rev,
#                                      where $major, $minor and $rev are given by the installer.
#                                      A symbolic link to the actual installation directory will be created
#                                      in $host_agent_installer_prefix_dir/dynatrace.
#  $host_agent_installer_file_name  => The file name of the Dynatrace Host Agent installer in the module's files directory.
#  $host_agent_installer_file_url   => A HTTP, HTTPS or FTP URL to the Dynatrace Web Host Agent installer
#                                      in the form (http|https|ftp)://[user[:pass]]@host.domain[:port]/path.
...
#  
#  $update_file_url                 => URL to the update zip file with tds inside e.g.
#                                      'https://files.dynatrace.com/downloads/fixpacks/dynaTrace-6.5.1.1003.zip'
...
class dynatraceappmon::params {
  ...
  case $::kernel {
    'Linux': {
      ...
      $dynatrace_collector        = 'dynaTraceCollector'
      $dynatrace_host_agent       = 'dynaTraceHostagent'
      $dynatrace_analysis         = 'dynaTraceAnalysis'
      $dynatrace_server           = 'dynaTraceServer'
      $dynatrace_webserver_agent = 'dynaTraceWebServerAgent'
      $dynatrace_frontend_server  = 'dynaTraceFrontendServer'
      $dynatrace_backend_server   = 'dynaTraceBackendServer'

      $services_to_manage_array = [
        $dynatrace_server,
        $dynatrace_collector,
        $dynatrace_analysis,
        $dynatrace_webserver_agent,
        $dynatrace_host_agent,

        $dynatrace_backend_server,
        $dynatrace_frontend_server
        ]
